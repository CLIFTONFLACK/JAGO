...
@interface PDAppDelegate () <MONActivityIndicatorViewDelegate,FBLoginViewDelegate,MFMailComposeViewControllerDelegate,MFMessageComposeViewControllerDelegate>
{
    UIView *viewPopup;
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    [self performSelector:@selector(getAllContacts) withObject:nil afterDelay:0.2];
...
[[PDHelper sharedHelper]getAllContacts:^(NSArray *arrContact) {
   ...
   self.arrAllContactData=[[NSMutableArray alloc]init];
       self.arrAllContactData=[arrContact mutableCopy];
    NSLog(@"%@",   self.arrAllContactData);
    [self performSelector:@selector(callWebServices) withObject:nil afterDelay:0.2];
    
}];
    //self.arrAllContactData = nil;
}
...
//         PDAppDelegate* appDelegate = [UIApplication sharedApplication].delegate;
...
- (void)loginViewFetchedUserInfo:(FBLoginView *)loginView
    ...
    /*id,name,picture.type(large),email,location,gender,birthday,hometown,first_name,last_name*/
    ...
    NSLog(@"%@", [user objectForKey:@"picture"]);
...
-(void)callWebServices
{
    ...
    if([[FBSession activeSession].accessTokenData.accessToken isKindOfClass:[NSNull class]]||[FBSession activeSession].accessTokenData.accessToken.length == 0)
    {
        NSLog(@"accesstoken not fetch");
    }
    else
    {
    ...
    [[PDWebHandler sharedWebHandler] startRequestWithCompletionBlock:^(id response, NSError *error)
     {
         ...
         else
         {
             ...
             for (int j=0; j<[friendDetailArray count]; j++)
             {
                 ...
                 for (int i=0; i<[self.arrAllContactData count]; i++)
                 {
                     ContactsData *obj= [self.arrAllContactData objectAtIndex:i];
                     ...
                     {
                         [   self.arrAllContactData removeObjectAtIndex:i];
                         [   self.arrAllContactData insertObject:obj atIndex:i];
                         obj.hasFriend=1;
