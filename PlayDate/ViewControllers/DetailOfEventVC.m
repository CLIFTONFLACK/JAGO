...
-(void)LoadData
{
    ...
    //edit by tijender
    lblStatus.text = [[self.dictSlected_EventInfo valueForKey:@"status"]uppercaseString];
    
    NSString *strStatus=[self.dictSlected_EventInfo valueForKey:@"status"];
    
    if ([strStatus isEqualToString:@"accepted"]||[strStatus isEqualToString:@"now"])
    {
        lblStatus.textColor=[UIColor colorWithRed:127.0/255.0 green:206.0/255.0 blue:49.0/255.0 alpha:1.0];
        vwStatus.backgroundColor=[UIColor colorWithRed:127.0/255.0 green:206.0/255.0 blue:49.0/255.0 alpha:1.0];
        
    }
    else  if ([strStatus isEqualToString:@"pending"]) {
        lblStatus.textColor=[UIColor colorWithRed:145.0/255.0 green:49.0/255.0 blue:206.0/255.0 alpha:1.0];
        vwStatus.backgroundColor=[UIColor colorWithRed:145.0/255.0 green:49.0/255.0 blue:206.0/255.0 alpha:1.0];
        
    }
    else
    {   lblStatus.textColor=[UIColor colorWithRed:255.0/255.0 green:0/255.0 blue:0/255.0 alpha:1.0];
        
    }
    vwStatus.layer.cornerRadius=10;
    vwStatus.clipsToBounds=YES;
...
//    }
    ...
    [imgPlayer2 addGestureRecognizer:gestureImgVwGuardianFrndChildInfo];
...
//    
    ...
    NSArray *arrAllergies=[[self.dictSlected_EventInfo valueForKey:@"friend_allergies"] componentsSeparatedByString:@","];
    NSMutableString *strAllergies=[[NSMutableString alloc]init];
...
//    
    ...
    lblChildInfo.text = @"CHILD'S FRIEND INFO";
    lblName.text=[self.dictSlected_EventInfo valueForKey:@"friendname"];
    lblDob.text=[[self.dictSlected_EventInfo valueForKey:@"friend_dob"] uppercaseString];
//    lblFreetime.text=[[[arrTemp objectAtIndex:ges.view.tag] valueForKey:@"friend_c_set_fixed_freetime"]uppercaseString];
    lblAllergies.text=strAllergies;
//    lblHobbies.text=strHobbies;
    lblYouthclub.text=[self.dictSlected_EventInfo valueForKey:@"friend_youth_club"];
