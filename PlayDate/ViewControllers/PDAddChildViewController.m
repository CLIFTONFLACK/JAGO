...
-(void)textfieldRadius
{
    tFDOB.layer.cornerRadius = 5.0f;
    tFDOB.layer.borderWidth = 2.0f;
    tFDOB.layer.borderColor = [UIColor whiteColor].CGColor;
    UIColor *color = [UIColor whiteColor];
    tFDOB.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"DATE OF BIRTH" attributes:@{NSForegroundColorAttributeName: color}];
    
    tFName.layer.cornerRadius = 5.0f;
    tFName.layer.borderWidth = 2.0f;
    tFName.layer.borderColor = [UIColor whiteColor].CGColor;
    
    UIColor *color1 = [UIColor whiteColor];
    tFName.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"CHILD FIRST NAME" attributes:@{NSForegroundColorAttributeName: color1}];
    
    tLName.layer.cornerRadius = 5.0f;
    tLName.layer.borderWidth = 2.0f;
    tLName.layer.borderColor = [UIColor whiteColor].CGColor;
    
    UIColor *color2 = [UIColor whiteColor];
    tLName.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"CHILD LAST NAME" attributes:@{NSForegroundColorAttributeName: color2}];
    
    vwAllergies.layer.cornerRadius = 5.0f;
    vwAllergies.layer.borderWidth = 2.0f;
    vwAllergies.layer.borderColor = [UIColor whiteColor].CGColor;
}
...
//    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES[C] %@", stringOnlyRegex];
    ...
    // VALIDATE NAME
    NSString *strFName = tFName.text;
    strFName = [strFName stringByReplacingOccurrencesOfString:@" " withString:@""];
   // if (strFName.length>0 && [predicate evaluateWithObject:strFName])
         ...
         {
             [[[UIAlertView alloc] initWithTitle:nil message:@"Please enter child name." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil] show];
             return NO;
         }
...
-(void)activityDidAppear
{
    ...
    NSString *strAppendFirstLastName;
    
    if(tLName.text.length >0)
    {
    NSString *str = tFName.text;
    strAppendFirstLastName = [str stringByAppendingFormat:@" %@",tLName.text];
    NSLog(@"%@",strAppendFirstLastName);
    }
    else
    {
        strAppendFirstLastName = tFName.text;
    }
    ...
    [params setObject:strAppendFirstLastName forKey:FBName];
...
//                [[[UIAlertView alloc] initWithTitle:@"" message:@"Child added successfully." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil] show];
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"Child added successfully." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                alert.tag = 101;
                [alert show];
                ...
                [btnProfilePic setImage:[UIImage imageNamed:@"placeholder"] forState:UIControlStateNormal];
                ...
                lblAllergies.textColor = [UIColor whiteColor];
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    if (textField == tFDOB) {
        ...
        [self resignKeyBoard];
        CGPoint scrollPoint = CGPointMake(0.0, textField.frame.origin.y+120.0);
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    CGPoint scrollPoint = CGPointMake(0.0, textField.frame.origin.y+70.0);
    [scrollView setContentOffset:scrollPoint animated:YES];
...
//    tFYouthClub.frame=rect;
  ...
  //  infoBackgroundView.frame=rect;
    ...
    rect =infoBackgroundView.frame;
  //  edit by tijender 25 to 125
    rect.size.height=CGRectGetMaxY(lblAllergies.frame)+125.0;
    infoBackgroundView.frame=rect;

    
    
    rect =btnSave.frame;
    rect.origin.y=CGRectGetMaxY(infoBackgroundView.frame)+37.0;
    btnSave.frame=rect;
   [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(btnSave.frame) + 20.0)];
}
...
//edit by tijender
//user_img to placeholder Xib
//txt_field_big image remove from textfield
@end
