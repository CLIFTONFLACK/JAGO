...
{
     NSLog(@"self.dictEventInfo_edit%@",self.dictEventInfo_edit);
...
{
    // image Round
    imgVwChildName.layer.cornerRadius = imgVwChildName.frame.size.height/2;
    imgVwChildName.clipsToBounds = YES;
    imgVwGuardianFrndChildName.layer.cornerRadius = imgVwGuardianFrndChildName.frame.size.height/2;
    imgVwGuardianFrndChildName.clipsToBounds = YES;
       ...
       NSLog(@"%@",strSetDate);
        ...
        if ([str_id isEqualToString:strGid])// if event onwer is same
        {
          ...
          [imgVwChildName setImageWithURL:[self.dictEventInfo_edit valueForKey:@"profile_image"] placeholderImage:[UIImage imageNamed:@"user_img"]];
            ...
            NSLog(@"str_Me_slected_Child_Id%@",str_Me_slected_Child_Id);
...
[scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(sendBtn.frame) + 80.0)];
    }
...
-(void)viewWillAppear:(BOOL)animated
{
    if (!firstClicked)
    {
        ...
        //edit by tijender
        rect.origin.y =CGRectGetMaxY(alternateButton.frame)-20.0;
        ...
        rect.origin.y =  CGRectGetMaxY(tVNotes.frame)+20;
        
        sendBtn.frame =rect;

        
        rect=containerView.frame;
        rect.size.height =CGRectGetMaxY(sendBtn.frame)+15.0;
        containerView.frame=rect;
        //edit by tijender 40 t0 80 size + scroll
              [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(sendBtn.frame) + 80.0)];
//        [self setUpViewContents];
        ///////////
    }
...
//    scrollView.frame = rect;
    //edit by tijender 40 to 80 size +
    [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(sendBtn.frame) + 80.0)];
}
...
-(void)setUpView
{
    ...
    rect.origin.y =CGRectGetMaxY(tVNotes.frame)+10;
    sendBtn.frame = rect;
    
    rect = containerView.frame;
   // rect.size.height = CGRectGetMaxY(tVNotes.frame)+10;
    rect.size.height = CGRectGetMaxY(sendBtn.frame)+10;
...
-(void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if (tfLocation==textView)
    {
        ...
        //edit by tijender 210 t0 310
            [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(sendBtn.frame) + 310.0)];
    ...
    else
    {
        ...
        //edit by tijender 180 to 280
        [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, CGRectGetMaxY(sendBtn.frame) + 280.0)];
...
-(void)changeContentsForLocation:(UITextView *)textvw
{
    ...
    if (textvw==tfLocation)
    {
        ...
        // containerView
        rectbtn.origin.y =  CGRectGetMaxY(tVNotes.frame)+20;
        ...
        rect = containerView.frame;
        //tVNotes
        rect.size.height = CGRectGetMaxY(sendBtn.frame) + 30.0;
        containerView.frame = rect;
...
-(void)Adjust_Size_height_EditInfo
{
    ...
    //rect.origin.y =CGRectGetMaxY(alternateButton.frame)+10.0;
    //edit by tijender
    rect.origin.y =CGRectGetMaxY(txtTemp.frame)+10.0;
    tfLocation.frame=rect;
...
-(void)Adjust_Size_height
{
   ...
   // rect.origin.y=CGRectGetMaxY(vwSchool.frame)+7.0;
      rect.origin.y=CGRectGetMaxY(vwAllergies.frame)+7.0;
    ...
    //37
   [scollvw setContentSize:CGSizeMake(scollvw.frame.size.width, CGRectGetMaxY(vwBackView.frame) + 80.0)];
}
    ...
    [[PDWebHandler sharedWebHandler] startRequestWithCompletionBlock:^(id response, NSError *error)
     {
         ...
         id result =response;
         NSLog(@"result%@",result);
         [result valueForKey:@"guardianchild"];
         
         NSMutableArray *uniqueChildIDs = [[NSMutableArray alloc] init];
         NSLog(@"uniqueChildIDs%@",uniqueChildIDs);
         ...
         if ([result valueForKey:@"success"])
         {
             ...
             [vwChildName setHidden:NO];
             [btnAddChild setHidden:YES];

         }
        else
        {
            NSLog(@"%@",error.description);
            [vwChildName setHidden:YES];
...
-(IBAction)btnAddChildAction:(id)sender
{
    ...
    [self.navigationController pushViewController:PDACVC animated:NO];
}
//edit by tijender
// create vwChildName
@end
