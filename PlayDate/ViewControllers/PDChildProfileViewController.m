...
- (void)viewDidLoad
{
    ...
    iVChildProfile.layer.cornerRadius = iVChildProfile.frame.size.height / 2;
    iVChildProfile.clipsToBounds = YES;
...
{
    [self resignKeyBoard];
    CGPoint scrollPoint = CGPointMake(0.0, lblFreeTime.frame.origin.y+90.0);
...
-(void)changeContentsForFreeTime
{
  ...
  //  conditionsBackgroundView.frame = rect;
    ...
    rect.origin.y = CGRectGetMaxY(dobBackgroundView.frame) + 5.0;
    //rect.origin.y = CGRectGetMaxY(freeTimeBackgroundView.frame) + 5.0;
...
//    youthClubBackgroundView.frame = rect;
   ...
   // schoolBackgroundView
    rect = btnSave.frame;
    rect.origin.y = CGRectGetMaxY(allergiesBackgroundView.frame) + 90.0;
    btnSave.frame = rect;
}
...
-(void)getChildInfo:(UITapGestureRecognizer *)Gesture
{
    ...
    NSLog(@"%@",arrParentData);
    objPDProfileViewController.strCheckId = self.strCheckId;
    objPDProfileViewController.index = self.indexForContactListFind;
