...

{
    ...
    NSArray *arrServerfriend;
    NSMutableArray *arrServerfriendfullDetail;
}
...
- (void)viewDidLoad
{
    ...
    NSLog(@"%d",self.index);
    NSLog(@"%@",self.strCheckId);
    [super viewDidLoad];
    arrServerfriendfullDetail = [[NSMutableArray alloc]initWithCapacity:1];
...
//         {
            ...
            arrServerfriendDetail= [response valueForKey:PDWebData];
             NSLog(@"%@",arrServerfriendDetail);

            arrServerfriend= [[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"];
             NSLog(@"%@",arrServerfriend);
             id authChild = [[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"childinfo"] ;
             childList = authChild;
             NSLog(@"childList%@",childList);
             
//             if ([result isKindOfClass:[NSArray class]]) {
                 NSMutableArray *arrTemp=[[NSMutableArray alloc]initWithCapacity:1];
             ...
             tFName.text =[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"name"];
             NSLog(@"%@",tFName.text);
          //   tFPhone.text=[[arrServerfriend objectAtIndex:0]objectForKey:@"phone"];
             NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
             [dateFormatter setDateFormat:@"yyyy-MM-dd"];
             NSString *dateString =[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"dob"];
             NSDate *date = [dateFormatter dateFromString:dateString];
             [dateFormatter setDateFormat:@"dd-MMMM-yyyy"];
             tFDOB.text = [[dateFormatter stringFromDate:date] uppercaseString];
             NSString *location = [[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"location"]uppercaseString];
             ...
             if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"m"])
             {
                 guardianType = @"MOTHER";
                 self.guardianType = G_MOTHER;
             }
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"gm"])
             {
                 guardianType = @"GRAND MOTHER";
                 self.guardianType = G_GRAND_MOTHER;
             }
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"gf"])
             {
                 guardianType = @"GRAND FATHER";
                 self.guardianType = G_GRAND_FATHER;
             }
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"b"])
             {
                 guardianType = @"BROTHER";
                 self.guardianType = G_BROTHER;
             }
             
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"s"])
             {
                 guardianType = @"SISTER";
                 self.guardianType = G_SISTER;
             }
             
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"n"])
             {
                 guardianType = @"NANNY";
                 self.guardianType = G_NANY;
             }
             else if  ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"t"])
             {
                 guardianType = @"TEACHER";
                 self.guardianType = G_TEACHER;
             }
             
             else if ([[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:@"guardian_type"] isEqualToString:@"o"])
             {
                 guardianType = @"OTHER";
         ...
         iVProfile.layer.cornerRadius = iVProfile.frame.size.height / 2;
         iVProfile.contentMode = UIViewContentModeScaleToFill;
         iVProfile.clipsToBounds = YES;
             tFUsers.text = guardianType;
             __block UIImageView *iV = iVProfile;
             __block UIView *iVC = profileImageViewContainer;
             [iVProfile setImageWithURL:[NSURL URLWithString:[[[arrServerfriendDetail objectAtIndex:self.index] objectForKey:@"friendinfo"] objectForKey:PDWebProfileImage]] placeholderImage:[UIImage imageNamed:@"user_img"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
                 ...
                 if (!error)
                 {
                    // CGSize perfectSize = [[PDHelper sharedHelper] perfectSizeOfImage:image withMaximumHeight:iV.frame.size.height andMaximumWidth:iV.frame.size.width];
                     CGSize perfectSize = CGSizeMake(160, 160);
...
-(void)infoShowCallWebService
{
    ...
    NSLog(@"str_Parent_gid%@",self.str_Parent_gid);
    ...
    [[PDWebHandler sharedWebHandler] startRequestWithCompletionBlock:^(id response, NSError *error)
     {
         ...
         dictParentInfo = [response objectForKey:@"parentinfo"];
          NSLog(@"parentinfo%@",dictParentInfo);
         id authChild = [response objectForKey:@"auth_child"];
         childList = authChild;
         NSLog(@"childList%@",childList);
          NSLog(@"authChild%@",authChild);
         if ([result isKindOfClass:[NSArray class]]) {
...
{
    PDAddChildViewController *PDACVC = [[PDAddChildViewController alloc]initWithNibName:@"PDAddChildViewController" bundle:nil];
    [self.navigationController pushViewController:PDACVC animated:YES];
...
-(void)reloadChilds
{
    //edit by tijnder if condition
    if([childList isKindOfClass:[NSNull class]])
    {
        [childScrollViewContainer setHidden:YES];
        [btnAddChild setHidden:NO];
    }
    else
    {
    // REMOVE ALL SUBVIEWS
    ...
    for (int i=0; i<[childs count]; i++)
    {
        ...
        NSLog(@"%@",[detail objectForKey:PDWebProfileImage]);
        ...
        [PDWebChildName capitalizedString];
         NSLog(@"%@",[detail objectForKey:PDWebChildName]);
        titleView.text = [[detail objectForKey:PDWebChildName] uppercaseString];
        if([self.strCheckId isEqualToString:@"FriendParentId"])
        {titleView.text = [[detail objectForKey:@"name"] uppercaseString];
        }
...
-(void)getChildInfo:(UITapGestureRecognizer *)gest
{
    ...
    NSLog(@"childs%@",childs);
    pdChildProfile.strCheckId = self.strCheckId;
    pdChildProfile.indexForContactListFind = self.index;
...
//[self.str_Parent_gid isKindOfClass:[NSNull class]]
@end
