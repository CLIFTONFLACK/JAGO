...
{
    NSLog(@"array%@",arrAllContactData);
    NSLog(@"appdelegate%@",[PDAppDelegate sharedDelegate].arrAllContactData);
    if([[PDAppDelegate sharedDelegate].arrAllContactData isKindOfClass:[NSNull class]] ||[[PDAppDelegate sharedDelegate].arrAllContactData count]== 0)
    {
        NSLog(@"nil array");
    ...
    {
        arrAllContactData = [PDAppDelegate sharedDelegate].arrAllContactData ;
        NSLog(@"array%@",arrAllContactData);
        NSLog(@"appdelegate%@",[PDAppDelegate sharedDelegate].arrAllContactData);
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSLog(@"arrServerfriendDetail===%@",arrServerfriendDetail);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   NSArray *parentInfo = [[arrServerfriendDetail objectAtIndex:indexPath.row] valueForKey:@"friendinfo"];
    NSLog(@"%@",parentInfo);
    
    PDProfileViewController *pdProfileView = [[PDProfileViewController alloc]initWithNibName:@"PDProfileViewController" bundle:nil];
    pdProfileView.strCheckId = @"FriendParentId";
    pdProfileView.index = indexPath.row;
    NSLog(@"%d",pdProfileView.index);
    [self.navigationController pushViewController:pdProfileView animated:YES];
    }
}
