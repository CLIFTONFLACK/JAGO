...
@interface PDLoginWithFBViewController ()
{
    ...
    IBOutlet UIView *termsConditionView;
...
//    [UIApplication sharedApplication]openURL:[NSURL URLWithString:termANDcondition];
}
...
-(IBAction)loginAction:(id)sender
{
    BOOL isAccepted = [[NSUserDefaults standardUserDefaults] boolForKey:@"TermsAndConditionAccepted"];
    if (isAccepted) {
...
- (IBAction)acceptAction:(id)sender {
    ...
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"TermsAndConditionAccepted"];
...
-(BOOL) webView:(UIWebView *)inWeb shouldStartLoadWithRequest:(NSURLRequest *)inRequest navigationType:(UIWebViewNavigationType)inType {
    ...
    if ( inType == UIWebViewNavigationTypeLinkClicked ) {
        NSLog(@"%@",[[inRequest URL]scheme]);
        if([[[inRequest URL]scheme] isEqualToString:@"http"])
        {
        NSString *iTunesLink = @"http://www.jago.nu/terms/";
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:iTunesLink]];
        }
        else if([[[inRequest URL]scheme] isEqualToString:@"https"])
        {
            NSString *iTunesLink = @"http://www.jago.nu/privacy-policy/";
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:iTunesLink]];
        }
